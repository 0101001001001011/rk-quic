...
порядке: существует ли вообще двоичный файл фреймворка
(`ls …/rk_quic.framework/rk_quic`); перечисляет ли `nm -gU` на нём
`_rk_quic_version`; есть ли срез симулятора наравне со срезом устройства.
